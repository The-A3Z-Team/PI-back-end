...
public interface PaymentRepository extends JpaRepository<Payment,Long> {
    ...
    @Query("SELECT p FROM Payment p WHERE p.idContinuingEducation = :idContinuingEducation AND YEAR(p.date) = :year")
    List<Payment> getPaymentsByIdContinuingEducationAndDateYear(
            @Param("idContinuingEducation") Long idContinuingEducation,
            @Param("year") int year
    );
